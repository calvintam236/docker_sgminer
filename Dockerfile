...
RUN apt-get update \
    && apt-get -y --no-install-recommends install lsb-release bzip2 ca-certificates curl git libcurl4-openssl-dev pkg-config libtool autoconf automake \
    ...
    && autoreconf -fi \
    && CFLAGS="-Os -Wall -I/opt/AMDAPPSDK-3.0/include -L/opt/amdgpu-pro/lib/x86_64-linux-gnu" ./configure --disable-adl --without-curses \
    && make \
    && make install \
    && cd .. \
    && rm -r sgminer/ \
